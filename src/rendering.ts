...
const DoggyGraphicsEngine = class {
	...
	render = (): void => {
		...
		var colorAttribLocation = this.gl!.getAttribLocation(
			this.program,
			"vertColor"
		);
		...
		this.gl!.drawArrays(
			this.gl!.TRIANGLES,
			0,
			triangleVertices32.length / lenPerRow
		);
		...
		colorAttribLocation = this.gl!.getAttribLocation(
			this.program,
			"vertColor"
		);
		...
		this.gl!.drawArrays(
			this.gl!.LINES,
			0,
			lineVertices32.length / lenPerRow
		);
		this.gl!.flush();
